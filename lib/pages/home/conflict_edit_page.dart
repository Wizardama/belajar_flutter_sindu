...
class ConlfictEditPage extends StatefulWidget {
  final Function refresh;
  final String uid;

  ConlfictEditPage({@required this.refresh, @required this.uid});

  @override
  _ConlfictEditPageState createState() => _ConlfictEditPageState();
}

class _ConlfictEditPageState extends State<ConlfictEditPage> {
  ...
  String _translateWilayah;
  int _selectedWilayah = 1;
  int _selectedBidang = 1;
  String _translateKategori;
  ...
  void _getConflictDetail() {
    _api
        .getKonflikDetail(token: _loginModel.token, uid: widget.uid)
        .then((response) {
      print(response.statusCode);
      if (response.statusCode == 200) {
        KonflikDetailModel konflik = KonflikDetailModel.fromJson(response.body);

        setState(() {
          _connectionProgres = false;

          _judul.text = konflik.judul;
          _selectedJenis = konflik.jenis == 1 ? 'K. Menonjol' : 'P. Konflik';
          _translateWilayah = konflik.namaWilayah;
          _selectedBidang = konflik.idBidang;
          _translateKategori = konflik.kategori;
          _tanggalPotensi.text = konflik.waktuPotensiKonflik;
          _uraian.text = konflik.uraianPotensiKonflik;
          _analisa.text = konflik.analisa;
          _prediksi.text = konflik.prediksi;
          _rekomendasi.text = konflik.rekomendasi;
          _selectedStatus =
              konflik.statusPenanggulangan == 0 ? 'Belum' : 'Tertangani';

          if (konflik.penanganan != null) {
            _fakta.text = konflik.penanganan.fakta;
            _keterangan.text = konflik.penanganan.keterangan;
          }

          if (konflik.laporan != null) {
            _nomor.text = konflik.laporan.nomor;
            _sumberInfo.text = konflik.laporan.sumberInformasi;
            _hubunganDenganSumber.text = konflik.laporan.hubunganDenganSumber;
            _caraMendapatkanInfo.text =
                konflik.laporan.caraMendapatkanInformasi;
            _tanggal.text = konflik.laporan.waktuMendapatkanInformasi;
            _nilaiInfo.text = konflik.laporan.nilaiInformasi;
            _perihal.text = konflik.laporan.perihal;
            _langkahIntelijen.text = konflik.laporan.langkahIntelijen;
          }
        });
      }
    }).catchError((error) {
  ...
  Widget _trioList() {
    if (SingletonModel.shared.filter != null) {
      ...
      // Translasi dulu wilayah sama kategori
      List<WilayahModel> listTWilayah = filter.wilayah.list
          .where((f) => f.namaWilayah == _translateWilayah)
          .toList();
      List<KategoriModel> listTKategori = filter.kategori.list
          .where((f) => f.namaKategori == _translateKategori)
          .toList();

      if (listTWilayah.length > 0) {
        setState(() {
          _selectedWilayah = listTWilayah[0].id;
        });
      }

      if (listTKategori.length > 0) {
        setState(() {
          _selectedKategori = listTKategori[0].id;
        });
      }
